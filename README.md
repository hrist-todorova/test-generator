...
* възможно е като последен параметър за зададем хеш с опциията за картинка : {picture: "path"}


`delete_question_by_text(question)` - изтриваме въпрос като зададем като подадем като параметър текстът му
...
* categories - хеш с категории указваш по колко въпроса желаем от всяка категория например {music: 2, history: 5}
